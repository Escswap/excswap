...
import { CurrencyAmount, BSC, SwapParameters, Token, Trade, TradeOptions, TradeType } from 'sb1swap-sdk'
...
export default function v1SwapArguments(trade: Trade, options: Omit<TradeOptions, 'feeOnTransfer'>): SwapParameters {
  ...
  const inputETH = trade.inputAmount.currency === BSC
  const outputETH = trade.outputAmount.currency === BSC
  if (inputETH && outputETH) throw new Error('BSC to BSC')
